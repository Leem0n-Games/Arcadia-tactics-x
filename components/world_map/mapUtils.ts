...
export const getBiomeNameForCoords = (q: number, r: number, dimension: Dimension): string => {
    ...
    if (dist > 50) return 'Archipiélago de las Islas Exteriores';
    if (r < -20) return 'El Norte Glacial y Picos Helados';
    if (r > 20) return 'Arenas Abrasadas de Zun';
    if (q < -15) return 'Bosques Susurrantes';
    if (q > 15) return 'Picos de Hierro y Minas';
    if (q > 5 && q < 15 && r > 5 && r < 15) return 'Ciénaga de las Penas';
