...
const GridFragmentShader = `
  ...
  void main() {
    ...
    if (vOverlayType < 0.5) {
      // 0.0 MOVEMENT RANGE TILE: Vibrant Royal Blue Matrix
      float pulse = sin(uTime * 3.5 - distFromCenter * 10.0) * 0.5 + 0.5;
      float scanline = sin((uv.x + uv.y) * 18.0 + uTime * 4.0) * 0.15;
      float fillAlpha = 0.55 + 0.25 * pulse + scanline;
      float borderAlpha = isBorder * 0.95 + isCorner * 0.40;
      alpha = max(fillAlpha, borderAlpha);
      finalColor = mix(vColor, vColor * 1.5 + vec3(0.2, 0.4, 0.8), pulse * 0.35);

    } else if (vOverlayType < 1.5) {
      // 1.0 ATTACK / SPELL RANGE TILE: Red Hazard or Violet Glow
      float pulse = sin(uTime * 4.5 - distFromCenter * 10.0) * 0.5 + 0.5;
      float stripe = step(0.5, fract((uv.x + uv.y) * 8.0 - uTime * 3.0));
      float fillAlpha = 0.52 + stripe * 0.22 + pulse * 0.15;
      float borderAlpha = isBorder * 0.95 + isCorner * 0.40;
      alpha = max(fillAlpha, borderAlpha);
      finalColor = mix(vColor, vColor * 1.4 + vec3(0.2, 0.2, 0.3), stripe * 0.25 + pulse * 0.25);

    } else if (vOverlayType < 2.5) {
      // 2.0 SELECTED / HOVERED TILE: Golden Target Core
      float fastPulse = sin(uTime * 8.0) * 0.5 + 0.5;
      float ring = smoothstep(0.35, 0.40, distFromCenter) - smoothstep(0.42, 0.47, distFromCenter);
      float fillAlpha = 0.65 + 0.30 * fastPulse + ring * 0.5;
      float borderAlpha = isBorder * 1.0;
      alpha = max(fillAlpha, borderAlpha);
...
}> = ({ mapData, validMoves, validTargets, attackRangeTiles = [] }) => {
