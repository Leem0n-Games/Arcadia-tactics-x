...
export const performEnemyAction = (
  ...
  if (aiBehavior === AIBehavior.SPELLCASTER) {
    ...
    if (dist > 1 && dist <= 6 && refreshedEnemy.stats.spellSlots.current > 0) {
      ...
      const totalSpell = roll.result + spellMod;
      ...
      if (store.triggerDiceRoll) {
        store.triggerDiceRoll({
          ...
          isCrit,
          isCritFail,
          formulaString: `d20(${roll.result}) + ${spellMod}(Dark Spell) = ${totalSpell} vs CA ${target.stats.ac}`,
  ...
  if (dist <= 1) {
    ...
    if (store.triggerDiceRoll) {
      store.triggerDiceRoll({
        ...
        formulaString: `${hasAdvantage ? 'Pounce (Ventaja)' : 'd20'}(${bestD20}) + ${hitMod} = ${totalHit} vs CA ${target.stats.ac}`,
