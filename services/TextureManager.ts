...
class UnifiedTextureManager {
  ...
  public resolveAssetUrl(keyOrUrl: string): string {
    ...
    // 4. Check TERRAIN mapping in ASSETS
    const terrainMap = ASSETS.TERRAIN as Record<string, string>;
    if (terrainMap[trimmed]) return terrainMap[trimmed];

    // 5. Check UNITS mapping in ASSETS
    const unitMap = ASSETS.UNITS as Record<string, string>;
    if (unitMap[trimmed]) return unitMap[trimmed];

    // 6. Check VOXEL_STRUCTURE_TEXTURES
    ...
    // 8. Smart fallback mapping for short filenames (e.g. 'skeleton.png', 'thunderer.png', 'paladin.png', 'grass.png')
    ...
    if (lower.includes('grass')) return ASSETS.TERRAIN[TerrainType.GRASS] || `${MC_BASE_URL}/grass_block_top.png`;
    if (lower.includes('forest')) return ASSETS.TERRAIN[TerrainType.FOREST];
    if (lower.includes('plains')) return ASSETS.TERRAIN[TerrainType.PLAINS];
  ...
  /**
   * Asynchronously loads a 2D HTMLImageElement with automatic local -> remote fallback.
  ...
  public get2DImage(keyOrUrl: string, onLoaded?: (img: HTMLImageElement | null) => void): HTMLImageElement | null {
    ...
    img.onerror = () => {
      // Direct Procedural Pixel Art Synthesis Fallback (0ms local generation, no 429/404 network errors)
      try {
