...
export const createBattleSlice: StateCreator<GameStore, [], [], BattleSlice> = (set, get) => ({
  ...
  startBattle: (terrain, weather, enemyId, customEnemyList, isBoss) => {
      ...
      const { updatedGrid: battleMap, voxelBlocks: voxelStructures } = generateVoxelDioramaFeatures(baseGrid, terrain, BATTLE_MAP_SIZE, dioramaSeed);
      ...
      for(let i=0; i<enemyCount; i++) { 
          ...
          if (isThisEnemyBoss) {
              scaledStats.hp = Math.floor(scaledStats.hp * 2.0); // Boss double HP
              scaledStats.ac += 2; // Boss extra AC
          }
      ...
      if (state.selectedAction === BattleAction.MOVE) {
          ...
          if (state.selectedAction === BattleAction.MAGIC && state.selectedSpell) {
               ...
               if (spell.type === SpellType.HEAL) {
                   ...
                   const totalSpellRoll = d20 + spellMod + profBonus;
                   const isCrit = d20 === 20;
                   ...
                   get().triggerDiceRoll({
                       ...
                       actionType: 'SPELL',
                       d20Roll: d20,
                       modifier: spellMod + profBonus,
                       total: totalSpellRoll,
                       targetAc: effectiveAc,
                       isHit,
                       isCrit,
                       isCritFail,
                       formulaString: `d20(${d20}) + ${spellMod + profBonus}(Spell) = ${totalSpellRoll} vs CA ${effectiveAc}`,
